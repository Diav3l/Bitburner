  ...
  while(true){//run forever
    ...
    for(let i = 0; i < hosts.length; i++){
      ...
      for(let i = 0; i < targets.length; i++){
        ...
        if(ns.getWeakenTime(current)>1.2e6 || ns.getGrowTime(current)>1.2e6 || ns.getHackTime(current)>1.2e6)
          continue;
    ...
    // constantly checks for new servers until all 25 are purchased
...
export function getGrowThreads(ns,availRam,growRam,current){
  ...
  let required = Math.ceil(ns.growthAnalyze(current,2))
...
export function getTargets(ns,masterHosts){
  ...
  for(let i = 0; i < masterHosts.length; i++)
	// && ns.getServerRequiredHackingLevel(masterHosts[i]) <= ns.getHackingLevel()
